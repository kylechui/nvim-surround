...
M.insert_surround = function(args)
    ...
    local lines = utils._get_lines(first_pos[1], last_pos[1])

    if args.mode == "V" then -- Visual line mode case (need to create new lines)
        -- Indent the inner lines
        lines = utils.indent_lines(lines)
        ...
        lines[#lines] = utils.insert_string(line, delimiters[2], last_pos[2] + 1)
        line = lines[1]
        lines[1] = utils.insert_string(line, delimiters[1], first_pos[2])
    end
    -- Update the buffer with the new lines
    utils._set_lines(first_pos[1], last_pos[1], lines)
end
...
M.delete_surround = function(args)
    if not args then
        ...
        M.delete_char = utils._get_char()
        if not M.delete_char then
            return
        end
        if not utils._is_valid_alias(M.delete_char) then
    ...
    local lines = utils._get_lines(left_sel.first_pos[1], right_sel.first_pos[1])
    -- Remove the delimiting pair
    lines[#lines] = utils.delete_string(lines[#lines], right_sel.first_pos[2], right_sel.last_pos[2])
    lines[1] = utils.delete_string(lines[1], left_sel.first_pos[2], left_sel.last_pos[2])
    -- Update the range of lines
    utils._set_lines(left_sel.first_pos[1], right_sel.first_pos[1], lines)
end
...
M.change_surround = function(args)
    ...
    lines[#lines] = utils.replace_string(lines[#lines], delimiters[2], right_sel.first_pos[2], right_sel.last_pos[2])
    lines[1] = utils.replace_string(lines[1], delimiters[1], left_sel.first_pos[2], left_sel.last_pos[2])
    -- Update the range of lines
    utils._set_lines(left_sel.first_pos[1], right_sel.first_pos[1], lines)
end
